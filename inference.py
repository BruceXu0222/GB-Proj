...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for i in range(X_RANGE):
        for j in range(Y_RANGE):
            allDomains.append((i,j))
            
    for character in variables:
        if (character == GHOST0 or character == GHOST1 or character == PAC):
            variableDomainsDict[character] = allDomains
        else:
            variableDomainsDict[character] = range(0, 8 + manhattanDistance((0,0), (X_RANGE - 1, Y_RANGE - 1)))
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for variable in eliminationOrder:
            unused_factors, joined_factor = joinFactorsByVariable(factors,variable)
            if len(joined_factor.unconditionedVariables()) > 1:
                joined_factor = eliminate(joined_factor, variable)
                unused_factors.append(joined_factor)
            factors = unused_factors
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            observation_prob = self.getObservationProb(observation, pacman_pos, pos, jail_pos)
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            ...
            for element in newPosDist:
                result[element] += newPosDist[element] * self.beliefs[pos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        avg_num_particles = self.numParticles // len(self.legalPositions)
        remaining = self.numParticles % len(self.legalPositions)
        self.particles = self.legalPositions * avg_num_particles

        for i in range(remaining):
            rand = random.randint(0, len(self.legalPositions) - 1)
            self.particles.append(self.legalPositions[rand])
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            ...
            new_list.append(distribution.sample())
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        all_ghost_positions = self.legalPositions
        all_perm = list(itertools.product(all_ghost_positions, repeat = self.numGhosts))
        random.shuffle(all_perm)
        num_particles = self.numParticles

        while (num_particles - len(all_perm) > 0):
            self.particles += all_perm
            num_particles  -= len(all_perm)
        
        self.particles += all_perm[1:num_particles]
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                prob *= self.getObservationProb(observation[i], pac_pos, particle[i], jail_pos)
            distribution[particle] += prob
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                thisAgent = self.ghostAgents[i]
                distribution = self.getPositionDistribution(gameState, oldParticle, i, thisAgent)
                current_sample = distribution.sample()
                newParticle[i] = current_sample

    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        distancer = self.distancer.getDistance

        most_likely_ghost_pos = []
        for ghost_distribution in livingGhostPositionDistributions:
            most_likely_ghost_pos.append(ghost_distribution.argMax())

        dist_from_pacman_to_ghost = {}
        for pos in most_likely_ghost_pos:
            dist_from_pacman_to_ghost[pos] = (distancer(pacmanPosition, pos))

        min_ghost_pos = min(dist_from_pacman_to_ghost, key=dist_from_pacman_to_ghost.get)
        ...
        for action in legal:
            ...
            curr_dist = distancer(successorPosition, min_ghost_pos)
